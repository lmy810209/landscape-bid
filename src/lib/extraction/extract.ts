...
- bid_method: 입찰 **심사 방식**을 우선 추출하세요. 단순 가격 기준 표현보다 심사 절차 표현이 우선입니다.
    * 1순위(키워드 발견 시 즉시 채택): "적격심사", "종합심사낙찰제", "종합평가", "수의계약", "협상에 의한 계약"
    * 2순위(1순위 표현이 전혀 없을 때만): "최저가" 등 가격 기준 표현
    * "최저가" 단어가 등장해도 본문 어딘가에 "적격심사 대상" 같은 문구가 있으면 반드시 "적격심사"로 추출하세요. (조경 유지관리 공사는 거의 적격심사이며, 최저가 낙찰제와 사정율 분포가 완전히 다릅니다.)
    * 본문 표현 그대로 사용. 추측 금지.
